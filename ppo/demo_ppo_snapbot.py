...
def main():
    ...
    for t in range(1, 2048 * 200+1):
        ...
        if t % n_step_per_update == 0 or done or n_step == 200:
            if n_step == 200:
                epi_cnt += 1
            if done:
                epi_cnt += 1
        ...
        if t % n_step_per_update == 0:
            ...
            if iter_cnt % 10 == 0:
                if not os.path.exists("./model"):
                    os.makedirs("./model")
                # torch.save(PPO.actor.state_dict(), f"./model/snapbot_actor_{iter_cnt}.pth")
                # torch.save(PPO.critic.state_dict(), f"./model/snapbot_critic_{iter_cnt}.pth")
